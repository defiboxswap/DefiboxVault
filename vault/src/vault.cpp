...
void vault::createcoll(const name &contract, const symbol &sym, const name &income_account, name fees_account, asset min_quantity, uint16_t income_ratio, uint16_t release_fees, uint16_t refund_ratio) {
   require_auth(ADMIN_ACCOUNT);
   ...
   symbol issue_symbol = symbol(symbol_code(string("S") + sym.code().to_string()), sym.precision());
   ...
   // 创建 SEOS token, 总发行量10亿，相同位精度
   uint32_t prec_num = pow(10, sym.precision());
   auto data = std::make_tuple(_self, asset(1000000000ULL * prec_num, issue_symbol));
   action(permission_level{_self, "active"_n}, STOKRN_ACCOUNT, "create"_n, data).send();

   auto logdata = std::make_tuple(new_id, contract, sym, issue_symbol, income_account, fees_account, min_quantity, income_ratio, release_fees, refund_ratio);
   action(permission_level{_self, "active"_n}, _self, "colupadtelog"_n, logdata).send();
}

void vault::updatecoll(uint64_t collateral_id, const name &income_account, name fees_account, asset min_quantity, uint16_t income_ratio, uint16_t release_fees, uint16_t refund_ratio) {
   require_auth(ADMIN_ACCOUNT);
   ...
   collaterals collateraltbl(_self, _self.value);
   auto itr = collateraltbl.require_find(collateral_id, "collateral not found");
   check(itr->deposit_symbol == min_quantity.symbol, "min_quantity symbol error");
   ...
   auto logdata = std::make_tuple(collateral_id, itr->deposit_contract, itr->deposit_symbol, itr->issue_symbol, income_account, fees_account, min_quantity, income_ratio, release_fees, refund_ratio);
   action(permission_level{_self, "active"_n}, _self, "colupadtelog"_n, logdata).send();
}
...
void vault::proxyto(name proxy) {
    ...
    action(
        ...
        EOSIO_ACCOUNT,
        name("voteproducer"),
        make_tuple(_self, proxy, std::vector<name>{} )
    ).send();
}
...
void vault::sellnext2(name owner, asset quantity, string memo) {
    ...
    if (owner != _self) {
      ...
      action(permission_level{_self, "active"_n}, EOS_TOKEN_ACCOUNT, "transfer"_n, data).send();
   ...
   if (_config.last_income_time > 0) {
      ...
      while (itr != collateraltbl.end()) {
         ...
         if (period > 0) {
            ...
            auto quantity = get_balance(itr->deposit_contract, itr->income_account, itr->deposit_symbol) * total_ratio / 10000L;
      
            // transfer to self
            if (quantity.amount > 0) {
               auto data = std::make_tuple(itr->income_account, _self, quantity, string("award"));
               action(permission_level{itr->income_account, "active"_n}, itr->deposit_contract, "transfer"_n, data).send();
            }
            // save
...
// logs
void vault::colupadtelog(uint64_t collateral_id, const name &deposit_contract, const symbol &deposit_symbol, const symbol &issue_symbol, const name &income_account, name fees_account, asset min_quantity, uint16_t income_ratio, uint16_t release_fees, uint16_t refund_ratio) {
   require_auth(_self);
}

void vault::depositlog(uint64_t collateral_id, const name &owner, const asset &quantity, uint64_t rate, block_timestamp time) {
   require_auth(_self);
}

void vault::releaselog(uint64_t log_id, uint64_t collateral_id, const name &owner, const asset &quantity, uint64_t rate, block_timestamp time) {
   require_auth(_self);
}

void vault::withdrawlog(uint64_t log_id, uint64_t collateral_id, const name &owner, const asset &withdraw_quantity, const asset &withdraw_award_fees, const asset &withdraw_sys_fees, const asset &refund_award_quantity, const asset &refund_sys_quantity, block_timestamp time) {
   require_auth(_self);
}
...
void vault::on_tokens_transfer(name from, name to, asset quantity, string memo) {
   if (from == _self || to != _self || from == ADMIN_ACCOUNT || from == EOSIO_ACCOUNT  || from == EOS_REX_ACCOUNT) {
      return;
   }
...
void vault::transfer_token_to(name contract, name to, asset quantity, string memo) {
   ...
   if (contract == EOS_TOKEN_ACCOUNT && quantity.symbol == EOS_SYMBOL) {
      ...
      if (quantity > balance) {
         // 取回的抵押物小于可用余额，则取回对应可解锁REX数量
         // 得到eos差额
         auto diff_eos = quantity - balance;
         print_f("transfer to % quantity %, balance % sellrex: % (%)\n", to, quantity, balance, diff_eos);
         balance.amount = 0;
         diff_eos.amount += 1; // 防止误差
         // 卖出得到对应的EOS
...
void vault::do_deposit(s_collateral collateral, const name &owner, const asset &quantity) {
   ...
   auto is_eos = collateral.deposit_contract == EOS_TOKEN_ACCOUNT && collateral.deposit_symbol == EOS_SYMBOL;
   uint64_t rate = is_eos ? get_eos_rate(quantity.amount * -1) : get_rate(collateral.deposit_contract, quantity * -1);
   print_f("rate: %, ", rate);
   ...
   action(permission_level{_self, "active"_n}, STOKRN_ACCOUNT, "issue"_n, data).send();

   // deposit
   auto data2 = std::make_tuple(collateral.id, owner, quantity, (uint64_t)rate, current_block_time());
   action(permission_level{_self, "active"_n}, _self, "depositlog"_n, data2).send();

   // buy rex
   if (is_eos) {
      action(permission_level{_self, "active"_n}, _self, "buyallrex"_n, std::make_tuple()).send();
   }
}
...
void vault::do_withdraw(s_collateral collateral, const name &owner, const asset &quantity) {
   ...
   auto is_eos = collateral.deposit_contract == EOS_TOKEN_ACCOUNT && collateral.deposit_symbol == EOS_SYMBOL;
   uint64_t rate = is_eos ? get_eos_rate(0) : get_rate(collateral.deposit_contract, asset(0, collateral.deposit_symbol));
   print_f("rate: %, ", rate);
   ...
   auto data = std::make_tuple(release_id, collateral.id, owner, quantity, rate, block_timestamp(etime));
...
void vault::withdraw_sellrex(name user, asset sell_quantity, asset tsf_quantity, string memo) {
   ...
   // 需要计算已到期的可用REX
   static uint64_t matured_rex = rexbal_it == rexbal_table.end() ? 0 : calculate_matured_rex(rexbal_it);
   print_f("matured_rex %\n", matured_rex);
   ...
   action(
      ...
      EOSIO_ACCOUNT,
      name("withdraw"),
      make_tuple(_self, sell_quantity)
   ).send();

   // 给用户提现，需要��下一个action，否则eos没有到账
   action(
      permission_level{_self, "active"_n},
      _self,
      name("sellnext"),
...
void vault::check_for_released(const name &owner) {
   ...
   if (itr != releasetbl.end()) {
      ...
      uint128_t rate1 = is_eos ? get_eos_rate(0) : get_rate(collateral.deposit_contract, asset(0, collateral.deposit_symbol));
      // print_f("rate0: % , rate1: % \n", rate0, rate1);
      if (rate1 < rate0) {
         rate1 = rate0;
      }
      ...
      action(permission_level{_self, "active"_n}, STOKRN_ACCOUNT, "retire"_n, data1).send();

      // 返回eos
      auto withdraw_quantity = asset(refund0_amount, collateral.deposit_symbol);
      // print_f("withdraw_quantity: %\n", withdraw_quantity);
      auto withdraw_fees = asset(refund0_amount * collateral.release_fees / 10000, collateral.deposit_symbol);

      withdraw_quantity -= withdraw_fees;
      if (withdraw_quantity.amount > 0) {
         transfer_token_to(collateral.deposit_contract, owner, withdraw_quantity, string("withdraw"));
      }
      ...
      if (withdraw_to_award_fees.amount > 0) {
         transfer_token_to(collateral.deposit_contract, collateral.income_account, withdraw_to_award_fees, string("withdraw fees"));
      }
      if (withdraw_to_sys_fees.amount > 0) {
         transfer_token_to(collateral.deposit_contract, collateral.fees_account, withdraw_to_sys_fees, string("withdraw fees"));
      }
      

      auto refund_quantity = asset(refund1_amount - refund0_amount, collateral.deposit_symbol);
      ...
      if (refund_to_award_quantity.amount > 0) {
         transfer_token_to(collateral.deposit_contract, collateral.income_account, refund_to_award_quantity, string("refund"));
      }
      if (refund_to_sys_quantity.amount > 0) {
         transfer_token_to(collateral.deposit_contract, collateral.fees_account, refund_to_sys_quantity, string("refund"));
      }
      ...
      auto data = std::make_tuple(log_id, collateral.id, owner, withdraw_quantity, withdraw_to_award_fees, withdraw_to_sys_fees, refund_to_award_quantity, refund_to_sys_quantity, current_block_time());
      action(permission_level{_self, "active"_n}, _self, "withdrawlog"_n, data).send();
   }
}